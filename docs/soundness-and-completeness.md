...
| Navigation profile | Symbols, references, semantic edges, includes, callsites, targets, embeddings, and search ranking remain available with explicit per-TU coverage. | CFG, data-flow, summaries, and bindings are intentionally unavailable; deep tools return structured coverage instead of an empty-complete result. | Missing deep rows in a navigation index prove that no control/data flow exists. |
